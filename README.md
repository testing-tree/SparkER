...
- **Custom routing:** select any relationship to reveal side connection dots on both entities AND a draggable waypoint handle on the line. Click a dot to change which side the line exits or enters from; drag the waypoint to freely reposition the bend within the orthogonal rectangle. All custom indicators turn <span style="color:#059669">**green**</span>. Double-click a side dot or the waypoint handle to reset that element to auto. The Properties panel also provides a reset option for sides. Entity movement preserves all custom settings.
  > Custom routing gives you full control over line layout, but may produce routings that the auto mode would avoid (e.g. visually awkward paths). Adjust manually or restore auto-routing if needed.
...
| Side connection dots | Connected side / Switch to top, right, bottom, left — green when custom, double-click to reset |
...
| Relationship line (selected) | Auto (blue) or custom routing active (green) |
...
- **Side:** which side of the entity box the line connects to. By default, the side is chosen automatically. Click one of the connection dots that appear when the relationship is selected to override. Active dots turn <span style="color:#059669">**green**</span> in custom mode. Double-click the green dot or click the side name in the Properties panel to restore auto.

**Waypoint:** a draggable circle always appears on the line when the relationship is selected. Drag it freely within the rectangle between the two ARM endpoints; the path stays fully orthogonal. The handle and selected line turn <span style="color:#059669">**green**</span> when a waypoint is set. Double-click the handle to clear it. Moving either entity box preserves all routing settings.
