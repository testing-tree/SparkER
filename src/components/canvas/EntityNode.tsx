  ...
  return (
    ...
    >
      ...
      {activeSide !== null && SIDES.map(pos => {
        ...
        const tip = isActive ? (hasPrefSide ? 'Custom side — double-click to reset' : 'Connected side') : `Switch to ${sideValue}`
        return (
          <div
            key={`ind-${pos}`}
            className={posClass}
            ...
            onDoubleClick={isActive && hasPrefSide ? (e) => {
              e.stopPropagation()
              updateRelationshipEnd(selectedRelId!, edgeEndKey!, { preferredSide: undefined })
            } : undefined}
          />
        )
      })}
